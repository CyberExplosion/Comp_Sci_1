...
/*********************
Takes an istream that contains a single postfix expression p and returns a string representing the infix equivalent of p
*********************/
string postfixEspr(istream& in) {
	...
	int number;
	//Used to remove infix expression from the stack
	string lExpr, rExpr;

	ch = in.peek();
	while (ch != EOF) {
		//If we have a whitespace character skip it and continue with the next iteration of this loop
		if (isspace(ch)) {
			ch = in.get();
			ch = in.peek();
			continue;
		}
		//Non space character is next in input stream 
		//If the next character is a number, read it, convert to string, and push the string onto the infix stack
		if (isdigit(ch)) {
			in >> number;
		...
		//If the next character is an operator, pop the two top infix expressions stored on the stack, put the operator between the two infix expressions, and the push the result on the stack
...
int main() {
	...
	while (input.size() != 0) {
		...
		cout << "The infix equivalent is " << postfixEspr(inputExpr) << endl;
